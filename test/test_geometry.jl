...
@testset "Enmap geometry" begin
    for W in (CarClenshawCurtis{Float64},) #(Pixell.WCS.WCSTransform, CarClenshawCurtis{Float64})
...
@testset "sky2pix every single pixel" begin
    box = [10   -10;           # RA
        -5     5] * degree  # DEC
    shape, wcs = geometry(CarClenshawCurtis{Float64}, box, 1 * degree)
    m = Enmap(ones(shape), wcs)
    for i in 1:shape[1]
        for j in 1:shape[2]
...
@testset "nonallocating WCS info utilities" begin
    shape, wcs = fullsky_geometry(CarClenshawCurtis{Float64}, deg2rad(1))
...
@testset "pixel skyareas" begin
    for W in (CarClenshawCurtis{Float64},)#(Pixell.WCS.WCSTransform, CarClenshawCurtis{Float64})
        ...
        box = [10   -10;           # RA
               -5     5] * degree  # DEC
        shape, wcs = geometry(W, box, (1/60) * degree)
...
@testset "pixareamap" begin
    ...
    fullgeom = fullsky_geometry(CarClenshawCurtis{Float64}, π/180)
